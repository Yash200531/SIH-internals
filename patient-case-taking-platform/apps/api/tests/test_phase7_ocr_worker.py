"""Leased OCR queue worker tests."""

from uuid import uuid4

import pytest

from app.documents.ocr_worker import ReadyDocument, run_once
...
async def test_worker_isolates_one_document_failure_and_continues_batch() -> None:
    first = ReadyDocument(uuid4(), uuid4(), 7)
    second = ReadyDocument(uuid4(), uuid4(), 7)
...
@pytest.mark.asyncio
async def test_ocr_kill_switch_does_not_claim_or_process_work() -> None:
    queue = _Queue([ReadyDocument(uuid4(), uuid4(), 7)])
